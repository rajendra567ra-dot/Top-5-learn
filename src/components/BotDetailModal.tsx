...
export const BotDetailModal: React.FC<BotDetailModalProps> = ({ 
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="p-4 sm:p-5 border-b border-slate-200 bg-white flex items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-1.5 flex-wrap justify-end">
            {/* Direct Pause / Resume Toggle inside Modal */}
            {onToggleBotStatus && (
              <button
                id="modal-toggle-bot-status-btn"
                type="button"
                onClick={() => onToggleBotStatus(bot.id)}
                title={bot.status === 'PAUSED' ? 'Resume Bot Execution' : 'Pause Bot Execution'}
                className={`px-3 py-1.5 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all border shadow-2xs cursor-pointer ${
                  bot.status === 'PAUSED'
            ...
            {/* Direct Delete Bot Button inside Modal */}
            {onDeleteBot && (
              <button
                id="modal-delete-bot-btn"
                type="button"
                onClick={() => setIsDeleteConfirmOpen(true)}
                title={`Delete ${bot.serialNumber} (${bot.name})`}
                className="px-2.5 py-1.5 rounded-xl text-xs font-semibold bg-rose-50 text-rose-700 border border-rose-200 hover:bg-rose-100 flex items-center gap-1.5 transition-all cursor-pointer shadow-2xs"
              >
        ...
        {/* Modal Footer */}
        <div className="p-3.5 sm:p-4 border-t border-slate-200 bg-white flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-2 text-xs font-medium">
            {bot.status === 'PAUSED' ? (
              <>
                <span className="w-2 h-2 rounded-full bg-amber-500"></span>
                <span className="text-amber-800">Bot Status: <strong>PAUSED (EXECUTION HALTED)</strong></span>
              </>
            ) : (
              <>
                <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
                <span className="text-slate-600">Bot Status: <strong className="text-emerald-700">ACTIVE & SCANNING</strong></span>
              </>
            )}
          </div>
      ...
      {/* Delete Confirmation Modal inside BotDetailModal (In-App, No window.confirm) */}
      {isDeleteConfirmOpen && (
        <div 
          ...
          >
            <div className="flex items-start gap-3.5">
              ...
              <div className="flex-1">
                <h3 className="text-base font-bold text-slate-900">
                  Delete {bot.serialNumber}?
                </h3>
                <p className="text-xs font-medium text-slate-600 mt-0.5">
                  {bot.name} • {bot.strategyCategory.replace('_', ' ')}
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="p-3.5 rounded-xl bg-rose-50/70 border border-rose-200 text-xs text-rose-900 space-y-1.5">
              ...
              <p className="text-[11px] text-rose-700">
                • Current Portfolio Balance: <strong>${bot.portfolioBalance.toFixed(2)}</strong> ({bot.totalPnL >= 0 ? '+' : ''}${bot.totalPnL.toFixed(2)})
              </p>
              <p className="text-[11px] text-rose-700">
                • Any active trades for {bot.serialNumber} will be immediately closed and cancelled.
              </p>
            </div>
